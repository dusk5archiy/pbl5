...
export function AuctionModal(props: AuctionModalProps) {
  ...
  const buttonClassName = "px-[1cqw] py-[0.5cqw] rounded bg-(--bg-color) border-2 border-white disabled:text-white whitespace-nowrap";
  ...
  return (
    <div className="w-full h-full flex flex-col gap-[1vw]">
      ...
      <div className="w-full flex flex-col">
        <div className="w-full flex justify-center whitespace-nowrap text-[4cqw]">{text}</div>
        <div className="w-full flex flex-row justify-evenly items-center">
          <div className="flex-1 flex justify-center whitespace-nowrap text-[6cqw]">{formatBudget(current_price)}</div>
          <div className="flex-1 flex justify-center whitespace-nowrap text-[6cqw]">→</div>
          <div className="flex-1 flex justify-center whitespace-nowrap text-[6cqw]">{(price != null && price != 0) ? formatBudget(current_price + price) : ""}</div>
        </div>
      </div>
      <div className="w-full flex flex-col gap-[1.5vw]">
        <div className="w-full grid grid-cols-3 grid-rows-2 justify-center gap-[1vw] text-[5cqw] p-[1vw]">
          {
            prices.map((p) =>
              <button
                key={p}
