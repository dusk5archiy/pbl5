...
class S7DatasetDiceClassificationConfig(BaseModel):
    image_resolution: tuple[int, int] = (32, 32)


class S7DatasetDiceClassification:
    def __init__(
        self,
    ...
    def _process_dice_crop(self, crop_data: DiceCrop):
        ...
        input_pil_image = process_pil_image(
            input_pil_image, self.config.image_resolution
        )
        for img, _, _ in generate_rotate_and_flip_images(input_pil_image):
            ...
            results.append((np.array(img_adjusted), crop_data.score - 1))
